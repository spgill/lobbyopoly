...
Samuel's Master Build-script V 1.1
...
gulp.task('watch', function() {
    ...
    // Make sure electron is actually asked for
    if (args.electron) {
    ...
    for (let name in tasks_js) {
        ...
        gulp_watch(globs.task_js.watch(name), function(ev) {
            gulp_util.log(`"${name}" triggered by "${ev.path}"`)
            gulp.start(name)
        })
    }
    ...
    for (let name in globs) {
        if (globs[name].watcher) {
            ...
            gulp_watch(globs[name].src, function(ev) {
                gulp_util.log(`"${globs[name].watcher}" triggered by "${ev.path}"`)
    ...
    for (let name of statics) {
        ...
        gulp_watch(`${directories.source}/${name}/**/*.*`, function(ev) {
            gulp_util.log(`"static" triggered by "${ev.path}"`)
            gulp.start('static')
        })
    }
})
